...
- 이미 커뮤니티에 발행된 AI 소식도 관리자 수정 화면에서 SNS 게시를 새로 요청할 수 있다. Instagram·Threads별
  기존 게시 이력이 없는 플랫폼만 요청을 생성하므로 이미 요청했거나 게시한 플랫폼에는 중복 게시하지 않는다.
- 실패한 플랫폼만 작성자 또는 관리자가 수동 재시도할 수 있다. 성공했거나 최초 저장 때 요청하지 않은 플랫폼은 사용자 수정 화면에서 선택할 수 없다.
- 관리자는 `PUBLISHED` 또는 `EXTERNALLY_DELETED` 이력에서 재등록을 요청할 수 있다. 기존 이력과
  사용자 요청 정보는 변경하지 않고, 사용자 게시 이력에 노출되지 않는 별도 관리자 게시 이력을 생성한다.
  중복 게시를 막기 위해 기존 게시물을 Instagram 또는 Threads에서 직접 삭제한 뒤 사용한다.
- 공개 SNS 허브는 재등록 이력을 포함해 동일 원본을 한 항목으로 묶고, 플랫폼별 최신 발행 링크만 표시한다.
...
- `V59`부터 SNS 게시 성공·실패·링크 확인 알림은 사용자에게 생성하지 않는다. 마이그레이션 적용 시
  기존 `SOCIAL_PUBLICATION` 알림도 삭제하고 알림 ENUM에서 해당 유형을 제거한다.

### 게시 이미지

- 최종 파일은 서버에서 `1080×1350` JPEG로 만든다.
- 리뷰 대표 이미지는 비율을 유지해 전부 보이도록 축소하고, 남는 영역은 블러 배경 없이 흰색으로 채운다.
- 리뷰 대표 이미지 하단 중앙에는 게시 언어에 맞춘 주류명을 현대적인 반투명 다크 패널과 흰색
  볼드 글씨로 합성한다. 긴 이름은 말줄임 없이 전부 표시하며, 최대 3줄을 우선으로 글꼴 크기를
  자동 조정하고 그래도 길면 줄 수를 늘린다.
- 에디션 주류의 이미지 자막은 `기본 이름 + 시리즈 식별자`까지만 사용하고 에디션 식별 값은 제외한다.
  에디션 식별 값은 주류명 위 왼쪽에 별도로 표시하고, 오른쪽에는 대표 이미지가 리뷰한 에디션과
  다를 수 있다는 안내를 게시 언어에 맞춰 표시한다. 게시 본문과 해시태그의 기존 전체 이름 표시는
  변경하지 않는다.
- 모든 SNS 이미지 좌상단에는 본문보다 작은 다크 라벨을 표시한다. 리뷰는 게시 언어에 따라 `후기` 또는
  `Review`, 일반 소식과 AI 소식은 최종 게시물에 저장된 말머리(일반·이벤트·출시 등)를 그대로 사용한다.
  말머리가 없는 기존 소식은 `일반`으로 표시한다.
- 리뷰 본문은 향·맛·피니시별 아로마 휠 항목과 테이스팅 노트를 함께 표시하고 총평은 제외한다.
  긴 리뷰는 한 항목이 뒤 항목을 밀어내지 않도록 각 본문에 사용할 수 있는 글자 수를 고르게 배분하며,
  아로마 휠 항목은 각각 최대 80자로 표시한다.
- 리뷰 제목은 `주류명 후기`로 표시한다.
- Instagram과 Threads의 리뷰 필수 구성은 동일하며 제목·링크·향/맛/피니시의 아로마와 테이스팅 노트·
  해시태그 순서로 표시한다.
- 리뷰·소식 등 모든 SNS 캡션은 플랫폼의 실제 최대치보다 여유를 두어 Instagram 1,800자,
  Threads 400자 이내로 생성한다.
- 리뷰 링크의 짧은 URL은 안내 문구 다음 줄에 표시하며 플랫폼 제한에서도 자르지 않는다.
- Instagram·Threads 리뷰에는 주류 제목 바로 다음에 `전체 리뷰는 프로필 링크에서 확인하세요 🔗`와
...
- 리뷰 해시태그는 사용자 입력 없이 `#카테고리 #주류이름 #캐바캐 #CaskByCask` 순서로 자동 생성한다.
  ...
  해시태그는 Instagram·Threads 본문 하단에 배치한다.
- 소식은 관리자가 등록한 배경과 텍스트를 합성하거나 이미지를 직접 업로드한다.
- 소식 배경과 직접 업로드 이미지의 권장 해상도는 `1080×1350px(4:5)`이며, 등록 화면의 이미지 편집기에서 4:5로 자른다.
- 배경 + 텍스트 모드에서는 선택한 배경과 입력 문구를 합성한 SNS 업로드 예시를 실시간으로 표시한다.
  썸네일 문구의 Enter 줄바꿈은 실제 생성 이미지에도 그대로 반영하며, 각 줄이 이미지 너비를 넘을 때만 자동 줄바꿈한다.
- 소식 본문 에디터에 삽입된 이미지는 문서 순서대로 SNS 캐러셀에 모두 포함한다. 대표 썸네일과 합쳐 플랫폼 한도인
  20장을 넘는 경우에는 본문 이미지 20장을 우선 게시한다.
- 소식 SNS 본문은 제목, 에디터 본문,
  `[자세한 내용은 CaskByCask(캐바캐) 홈페이지를 확인해주세요]`, 짧은 URL, 해시태그 순서로 구성한다.
  안내 문구는 대괄호를 포함해 짧은 URL 바로 위에 표시하고, 해시태그는 본문 맨 아래에 배치한다.
- 소식 본문의 줄바꿈은 웹 본문(`.notice-content`) 렌더링과 동일하게 SNS 캡션에도 반영한다.
  문단·목록 등 블록 태그와 `<br>`, `<p>` 내부 개행(`white-space: pre-wrap`)을 줄바꿈으로 변환하고,
  빈 문단은 빈 줄 1줄까지만 유지해 Threads 400자 게시 한도를 절약한다.
...
| `PUBLISHED` | 게시 완료 | permalink로 확인. 삭제 후 다시 올릴 때만 관리자 재등록 요청 |
| `EXTERNALLY_DELETED` | 플랫폼에서 직접 삭제됨 | 필요하면 관리자 재등록 요청 |
...
30분 뒤에도 링크를 얻지 못하면 상태는 `PUBLISHED`로 확정하되 오류 안내를 남긴다. 관리자 재등록 요청은
플랫폼에서 기존 게시물의 삭제 여부를 직접 확인한 뒤에만 사용한다.

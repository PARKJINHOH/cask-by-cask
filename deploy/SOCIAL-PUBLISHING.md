...
- 리뷰 대표 이미지는 비율을 유지해 전부 보이도록 축소하고, 남는 영역은 동일 이미지를 확대한 블러·딤 배경으로 채운다.
- 리뷰 본문은 테이스팅 노트를 제외하고 향·맛·피니시별 아로마 휠 항목을 각각 최대 80자로 표시한다.
- 총평은 최대 200자로 표시하되 링크 길이를 먼저 확보하고, 남은 플랫폼 글자 수가 부족하면 `...`으로 더 줄이므로 링크는 잘리지 않는다.
- 리뷰 링크는 URL만 노출하지 않고 `전체 리뷰 보기 → URL` 형식으로 안내한다.

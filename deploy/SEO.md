# DrinkIndex SEO/AEO 운영 가이드

운영 도메인 (`https://drinkindex.net`) 배포 후 수행해야 하는 검색 노출 작업 체크리스트.

---

## 1. 런칭 직전 점검

- [ ] DNS 설정 완료, HTTPS 정상 동작 (`curl -I https://drinkindex.net`)
- [ ] `https://drinkindex.net/robots.txt` 응답 200, 내용 정상
- [ ] `https://drinkindex.net/sitemap.xml` 응답 200, `<url>` 항목 포함
- [ ] `https://drinkindex.net/llms.txt` 응답 200
- [ ] 메인/카테고리/공지 등 주요 페이지의 HTML `<head>` 에 페이지별 `<title>`, `<meta name="description">`, `<link rel="canonical">`, JSON-LD 가 들어있는지 확인
  ...
  - 명령행: `curl -sL https://drinkindex.net/spirits | grep -E "<title>|description|canonical"`
...
2. **도메인 속성** (`drinkindex.net`) 또는 **URL 접두어** (`https://drinkindex.net/`) 선택
...
4. 좌측 메뉴 → **사이트맵** → `https://drinkindex.net/sitemap.xml` 제출
...
2. **사이트 관리** → 사이트 등록 → `https://drinkindex.net`
3. 소유권 확인 (HTML 파일 업로드 또는 메타 태그 방식)
4. **요청** → 사이트맵 제출 → `https://drinkindex.net/sitemap.xml`
5. **요청** → RSS 제출 (선택)
...
2. **Add a site** → `https://drinkindex.net`
...
### llms.txt
- `https://drinkindex.net/llms.txt` 응답 200 확인
...
`drinkindex-web/scripts/prerender.mjs` 가 build 시 정적 라우트들의 HTML 스냅샷을 dist/<route>/index.html 로 생성합니다.
...
1. `https://drinkindex.net/robots.txt` 확인 — 실수로 `Disallow: /` 안 들어갔는지
2. 주요 페이지 `curl -sL https://drinkindex.net/spirits | grep "noindex"` — noindex 안 박혔는지
3. Search Console "페이지" → 색인 제외 사유 확인 (예: "noindex로 차단됨", "표준 URL과 다름")
4. `sitemap.xml` 응답 200 확인, 마지막 lastmod 가 합리적인 날짜인지
5. SSL 인증서 만료 확인 (`openssl s_client -connect drinkindex.net:443 -servername drinkindex.net 2>/dev/null | openssl x509 -noout -dates`)

...
public class PriceReportImageServeController {
    ...
    public PriceReportImageServeController(
            @Value("${storage.local.base-path}") String basePathStr,
            PriceReportImageRepository priceReportImageRepository
    ) {
        this.basePath = Paths.get(basePathStr).toAbsolutePath().normalize();
        this.priceReportImageRepository = priceReportImageRepository;
    }

    /**
     * 가격 제보 이미지 서빙 (local 전용)
    ...
    public ResponseEntity<Resource> serveImage(@PathVariable String savedFileName) {
        ...
        PriceReportImage image = priceReportImageRepository.findBySavedFileName(savedFileName)
                .orElseThrow(() -> new CustomException(ErrorCode.PRICE_REPORT_IMAGE_NOT_FOUND));

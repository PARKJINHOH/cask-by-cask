...
public interface ReviewQueryRepository {
    ...
    /**
     * 마이페이지 "내 리뷰" 목록. 본인 리뷰 전체(숨김·비ACTIVE 주류 포함)를 카테고리·주류명
     * 키워드로 필터링해 페이징 조회한다. 공개 목록과 달리 노출 조건을 걸지 않는다.
     *
     * @param userId   본인 사용자 ID (필수)
     * @param category 주류 카테고리 필터. {@code null} 이면 전체
     * @param keyword  주류명(nameKo/nameEn) 부분 일치 검색어. 공백/{@code null} 이면 전체
     * @param sort     정렬 기준. {@code null} 이면 최신순
     * @param lang     이름 정렬에 쓸 표시 언어("en" 이면 nameEn 우선, 없으면 nameKo 폴백)
     */
    Page<Review> searchMyReviews(Long userId, SpiritCategory category, String keyword,

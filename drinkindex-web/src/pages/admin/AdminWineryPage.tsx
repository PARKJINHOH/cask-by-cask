...
import {
  ...
  useDeleteWinery,
  type WineryFilters,
} from '@/domain/admin/hooks/useAdminWinery'
...
function WineryForm({ initial, onSave, onCancel, isPending }: WineryFormProps) {
...
const EMPTY_FILTERS: WineryFilters = { nameKo: '', nameEn: '', country: '', foundedYear: '' }

export default function AdminWineryPage() {
  const [filterInput, setFilterInput] = useState<WineryFilters>(EMPTY_FILTERS)
  const [appliedFilters, setAppliedFilters] = useState<WineryFilters>(EMPTY_FILTERS)
  const [page, setPage] = useState(0)
  const [sort, setSort] = useState<SortState>({ field: 'id', dir: 'desc' })
  const [showCreate, setShowCreate] = useState(false)
  const [editTarget, setEditTarget] = useState<Winery | null>(null)
  const editFormRef = useRef<HTMLDivElement>(null)

  const { data, isLoading } = useAdminWineries(appliedFilters, page, `${sort.field},${sort.dir}`)
  ...
  const handleCreate = (form: FormValues) => {
    const payload: CreateWineryPayload = {
      nameKo: form.nameKo,
      nameEn: form.nameEn,
  ...
  const handleUpdate = (form: FormValues) => {
    ...
    const payload: UpdateWineryPayload = {
      nameKo: form.nameKo,
      nameEn: form.nameEn,
      country: form.country,
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-neutral-900">와이너리 관리</h1>
      ...
      {showCreate && (
        <div className="bg-white rounded-xl shadow-sm p-5 border border-primary-100">
          <h2 className="text-sm font-semibold text-neutral-700 mb-4">새 와이너리 등록</h2>
          <WineryForm
            onSave={handleCreate}
                ...
                {!data || data.empty ? (
                  ...
                  data.content.map((w) => (
                    <tr key={w.id} className="hover:bg-neutral-50 transition-colors">
                      <td className="px-4 py-3 text-neutral-400 tabular-nums">{w.id}</td>
                      <td className="px-4 py-3 font-medium text-neutral-900">{w.nameKo}</td>
                      <td className="px-4 py-3 text-neutral-500">{w.nameEn}</td>
                      <td className="px-4 py-3 text-neutral-500">{w.country}</td>
                      <td className="px-4 py-3 text-neutral-400">{w.region ?? '-'}</td>
                      <td className="px-4 py-3 text-neutral-400">{w.foundedYear ?? '-'}</td>
                      <td className="px-4 py-3 text-neutral-400">
                        {w.website
                          ? <a href={w.website} target="_blank" rel="noopener noreferrer" className="text-primary-800 hover:underline truncate max-w-[140px] inline-block">{w.website}</a>
                          : '-'}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex gap-1 justify-end">
                          <button
                            onClick={() => { setEditTarget(w); setShowCreate(false) }}
                          ...
                          <button
                            onClick={() => handleDelete(w.id, w.nameKo)}

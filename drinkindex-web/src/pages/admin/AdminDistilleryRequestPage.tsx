...
import {
  useAdminDistilleryRequests,
  useApproveDistilleryRequest,
  useRejectDistilleryRequest,
} from '@/domain/distillery/hooks/useDistilleryRequest'
import type { MyDistilleryRequest } from '@/domain/distillery/types/distilleryRequest.types'
...
interface RejectModalProps {
  open: boolean
  request: MyDistilleryRequest | null
  onClose: () => void
}

function RejectModal({ open, request, onClose }: RejectModalProps) {
  const [reason, setReason] = useState('')
  const reject = useRejectDistilleryRequest()
...
export default function AdminDistilleryRequestPage() {
  const [status, setStatus] = useState<RequestStatus>('PENDING')
  const [page, setPage] = useState(0)
  const [rejectTarget, setRejectTarget] = useState<MyDistilleryRequest | null>(null)
  const approve = useApproveDistilleryRequest()

  const { data, isLoading } = useAdminDistilleryRequests(status, page)

  const handleApprove = (req: MyDistilleryRequest) => {

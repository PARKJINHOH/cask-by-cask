...
interface Props {
  ...
  placeholder?: string
  /** 지정 시 해당 타입 생산자만 검색 (카테고리 게이팅) */
  type?: ProducerType
}

const DEBOUNCE_MS = 400

export default function ProducerSelector({ value, defaultName, onChange, placeholder = '생산자 검색...', type }: Props) {
  ...
  const { data: results = [] } = useProducers(searchQuery, type)

...
public interface RankingRepository extends Repository<User, Long> {
    ...
    String BASE_WHERE =
        "WHERE u.role != 'ADMIN' AND u.is_active = 1 AND u.deleted_at IS NULL ";
    String COUNT_QUERY =
    ...
    @Query(value = """
        ...
        WHERE maturing_power > :score AND role != 'ADMIN' AND is_active = 1 AND deleted_at IS NULL
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        LEFT JOIN (SELECT user_id, SUM(score) AS ws FROM score_history WHERE created_at >= :weekStart GROUP BY user_id) wk ON u.id = wk.user_id
        WHERE COALESCE(wk.ws, 0) > :score AND u.role != 'ADMIN' AND u.is_active = 1 AND u.deleted_at IS NULL
        """, nativeQuery = true)
    ...
    @Query(value = """
        SELECT COUNT(*) FROM users u
        LEFT JOIN (SELECT user_id, SUM(score) AS ms FROM score_history WHERE created_at >= :monthStart GROUP BY user_id) mo ON u.id = mo.user_id
        WHERE COALESCE(mo.ms, 0) > :score AND u.role != 'ADMIN' AND u.is_active = 1 AND u.deleted_at IS NULL
        """, nativeQuery = true)

...
1. 자동 수집 활성화 여부와 라이선스/worker 준비 상태 확인
...
라이선스 전 화면에서는 실제 Vivino 수집 버튼을 비활성화한다. 대신 합성 fixture 또는 Vivino가 승인한
샘플을 사용하는 `라이선스 검토 데모`만 실행할 수 있으며 누적 3건·로컬·HIDDEN 제한을 그대로 적용한다.
...
| 카드 | 표시 내용 |
|---|---|
| 공급자 | Fixture / 승인 샘플 / Vivino Licensed |
| 라이선스 | 미설정 / 제한 PoC / 정식 허가 / 설정 불일치 |
...
- 검색: 한글명, 영문명, external wine/vintage ID
- 결과 상태:
  - 전체
  - 등록 완료
  - 정보 갱신

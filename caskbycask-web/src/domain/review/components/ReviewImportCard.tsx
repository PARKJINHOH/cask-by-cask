...
/**
 * 다른 커뮤니티에 써 둔 자기 리뷰를 붙여넣거나 공개 게시글 링크로 불러와 폼을 채우는 카드.
 *
...
export default function ReviewImportCard({ spiritName, hasValues, onApply, onManualInput }: Props) {
    ...
    <div className="rounded-2xl border border-amber-200/70 bg-amber-50/40 p-4 space-y-3">
      ...
      >
        ...
        <span className="flex-shrink-0 text-xs text-neutral-500">
          {open ? t('review.import.collapse') : t('review.import.expand')}
        </span>
      </button>
      ...
      {open && (
        <div className="space-y-3">
          ...
          {tab === 'paste' ? (
            ...
            <div className="space-y-2">
              <input
                ...
                // 이 카드는 리뷰 폼 <form> 안에 있다. 막지 않으면 주소를 넣고 Enter 를 누른 순간
          ...
          {plan && plan.outcome !== 'ok' && (
            <BlockedNotice plan={plan} onManualInput={() => { setOpen(false); onManualInput() }} />
          )}

    ...
    <div className={`flex flex-col ${fill ? 'h-full' : ''}`}>
      ...
      */}
      {(onClose || onPrev || onNext) && (
        <div className="flex shrink-0 items-center gap-2 bg-neutral-950 px-3 py-2">
      ...
      {/*
        플레이어.
        유튜브는 iframe 이 <b>실제로 그려진 크기</b>를 보고 화질을 고른다(재생 화질을 지정하는
        파라미터·API 는 오래전에 무시되기 시작했다). 그래서 화질을 올리는 실질적인 방법은
        플레이어를 크게 두는 것뿐이다 — 여기서 폭을 좁히면 그만큼 화질이 내려간다.
      */}
      <div className="flex shrink-0 items-center justify-center bg-black">
        <div
          className={isShorts ? 'w-full max-w-[min(420px,52vh)]' : 'w-full'}
      ...
      {/* 정보 */}
      <div className={`min-h-0 flex-1 space-y-4 p-4 sm:p-5 ${fill ? 'overflow-y-auto' : ''}`}>
        <div>

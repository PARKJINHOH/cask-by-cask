      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-amber-50 text-neutral-600 text-left text-xs">
            {['category', 'name', 'batch', 'price', 'status'].map(k => (
              <th key={k} className="px-3 py-2 whitespace-nowrap font-medium">
                {t(`collection.table.${k}`)}
              </th>
            ))}
            {editable && <th className="px-3 py-2 text-center whitespace-nowrap min-w-20">{t('collection.table.visibility')}</th>}
            {editable && <th className="px-3 py-2 text-center whitespace-nowrap min-w-28">{t('collection.table.actions')}</th>}
          </tr>
        </thead>
        <tbody>
          {bottles.map(b => (
            <tr key={b.id} className="border-b hover:bg-neutral-50">
              ...
              <td className="px-3 py-2 font-medium min-w-40">{displayName(b)}</td>
              ...
              {editable && (
                <td className="px-3 py-2 whitespace-nowrap min-w-28">
                  <div className="flex flex-nowrap gap-2 justify-center">
                    <button onClick={() => onEdit?.(b)} className="text-neutral-400 hover:text-amber-600 text-xs px-1 whitespace-nowrap">{t('collection.editBottle')}</button>

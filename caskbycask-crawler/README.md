...
`wine_main.py`는 관리자에서 만든 실행 요청을 가져와 와인 마스터와 빈티지를 등록한다. 기본 모드는 외부 통신이 없는
`FIXTURE`이며, 이용 허가 검토용 가상 데이터 최대 3건만 `HIDDEN` 상태로 저장한다. LIVE는 서면 허가를
관리자 설정에 기록한 뒤 공개 Vivino HTML만 읽는 웹 크롤러다.

- `LIVE`는 관리자 설정에서 제공자 모드 `LIVE`, 웹 크롤링 허가 확인, 허가 근거 문구가 모두 저장된 경우에만 열린다.
- API URL·토큰·로그인 쿠키는 사용하지 않는다. 허가 범위의 공개 카탈로그/상세 HTML만 수집한다.
- 후보 시작 페이지는 `VIVINO_START_URLS`, 실행당 탐색 페이지 수는 `VIVINO_DISCOVERY_PAGE_LIMIT`, 요청 간격은
  `VIVINO_REQUEST_DELAY_SECONDS`로 설정하며 와인 상세 처리량은 실행당·시간당 최대 10건이다.
...
- 영문명은 허가된 Vivino 원문을 그대로 사용한다. 국문명 자동 검색·음차는 하지 않으며 관리자 검수 화면에서 수동 입력 후 공개한다.
...
로컬에서는 API를 실행한 뒤 `관리자 > 주류 > 와인 크롤링`에서 `샘플 3건 수집`을 누르고 다음 명령을 실행한다.

```bash
python wine_main.py
```

운영 cron은 `run-wine.sh`를 매시 37분에 호출한다. 라이선스 게이트가 닫혀 있으면 새 LIVE 실행을 만들지 않고,
관리자가 요청한 FIXTURE 실행만 처리한다.

...
Oracle Cloud에서 `run-news.sh`를 매시간 실행한다 — 실제 수집 주기는 관리자 설정이 정하고,
...
- 관리자가 정한 **수집 주기(시간, 기본 2)** 마다 **등록 출처 안에서만** 검색한다. 실행마다 주종 하나에
  집중하며(관리자 설정의 주종 비율이 순환 비중이다), 서버 설정 기본값은 하루 3건 **수집**이다.
...
- 핫딜은 짝수 시각 정각(`0 */2 * * *`), AI 소식은 매시간 17분(`17 * * * *`)에 실행해 Gemini 호출이 겹치지 않게 한다.
  AI 소식 cron 은 확인만 하고, 수집 여부는 관리자 설정의 수집 주기가 정한다.
...
- 출처 관리에서 **수집 활성**인 URL을 매 실행 때 확인하고, 결과를 관리자 목록에 기록한다.
  근거를 실제로 가져왔으면 `수집 성공`, 정상 확인했지만 새 소식이 없으면 `결과 없음`,
  확인 자체가 실패하면 사유와 함께 `수집 실패`다. 일반 웹사이트의 일시적인 연결 실패는 재시도하고,
  Instagram처럼 서버 직접 수집이 제한된 플랫폼은 Tavily 제한 검색으로만 확인한다.
  **검색이 성공해도 다른 출처의 실패를 덮지 않는다** — 예전에는 덮어써서 실패가 화면에 뜨지 않았다.

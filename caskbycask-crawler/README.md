...
Oracle Cloud에서 `run-news.sh`를 매시간 실행한다 — 실제 수집 시각은 관리자 설정이 정하고,
차례가 아니면 크롤러가 곧바로 종료한다.
...
```text
관리자가 등록한 출처에서 기사 목록 얻기 (피드 → 사이트맵 → 목록 링크, 유일한 발견 경로)
  → 각 기사를 열어 본문·발행일 수집 → 최신 기사 기간(기본 3일) 밖은 제외
  → Gemini 분류기가 제목·요약 생성 (실행마다 주종 하나를 우선)
...
- 관리자가 정한 **수집 시각(기본 `9,18` = 하루 두 번)** 에 **등록 출처만** 직접 확인한다.
  cron 은 매시간 17분에 확인하므로 실제 실행은 **09:17 · 18:17** 이다.
  실행마다 주종 하나를 우선해 소재를 고르며(관리자 설정의 주종 비율이 순환 비중이다.
  하루 2회면 한 바퀴가 5일), 서버 설정 기본값은 하루 3건 **수집**이다.
- 근거는 **개별 기사 URL** 이다. 목록 페이지를 근거로 넣으면 서버 중복 판정이 그 출처를 영구히 잠근다 —
  기사를 하나도 못 얻은 출처는 목록 URL 을 제출하지 않고 `결과 없음` 으로 남긴다.
...
- 핫딜은 짝수 시각 정각(`0 */2 * * *`), AI 소식은 매시간 17분(`17 * * * *`)에 실행해 Gemini 호출이 겹치지 않게 한다.
  AI 소식 cron 은 확인만 하고, 수집 여부는 관리자 설정의 수집 시각이 정한다.
- **자동 발행, AI 본문 작성, AI 이미지 생성을 모두 하지 않는다.**
- `관리자 > 커뮤니티 > 소식(AI)`에서 자동화·출처·쓸 거리·사용량을 관리한다.
- 출처 관리에서 **수집 활성**인 URL을 매 실행 때 확인하고, 결과를 관리자 목록에 기록한다.
  최근 기사를 실제로 가져왔으면 `수집 성공`, 목록은 정상인데 기간 안에 새 기사가 없으면 `결과 없음`,
  확인 자체가 실패하면 사유와 함께 `수집 실패`다.
  일반 웹사이트의 일시적인 연결 실패는 재시도한다. Instagram처럼 서버 직접 수집이 제한된 플랫폼은
  **확인할 방법이 없으므로 사유와 함께 `수집 실패`로 남긴다** — 조용히 넘기면 손쓸 수 없는 출처인 줄
  모른 채 새 소식을 기다리게 된다. 실패는 그 출처에만 남고 다른 출처의 결과를 덮지 않는다.
- 네이버 카페·디시인사이드 핫딜 대상은 별도 `targets.json`이 소유하며 AI 소식 출처 관리와 공유하지 않는다.
- 첫 배포는 `automationEnabled=false`이다.
- 필수 추가 환경변수는 `GEMINI_API_KEY` 하나이며 모델과 비용·토큰 절대 상한, 기사 수집 요청량 상한
  (`AI_NEWS_ARTICLES_PER_SOURCE`, `AI_NEWS_MAX_ARTICLES_PER_RUN`)은 `.env.example`을 참고한다.

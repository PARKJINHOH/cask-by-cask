...
`AiNewsService.resolveSourceScopeKey()` 는 **범위 판정 전용**이다. 행을 만들지 않는다.
...
- 유일한 발견 경로는 `collect_registered_sources()`(`news_official.py`) 의 `include_domains` 제한 검색이다.
  **여기에 도메인 무제한 검색을 다시 넣지 말 것** — 그게 문제의 출발점이었다.
...
| 상태 | 뜻 |
|---|---|
| `SUCCESS` | 이번 실행에서 이 출처에서 근거를 **실제로 가져왔다**(직접 확인 성공 또는 제한 검색 결과 매칭) |
| `NO_RESULT` | 확인은 정상이었지만 새 소식이 없었다. **실패가 아니다** |
| `ERROR` | 확인 자체가 실패했다. 사유를 `last_crawl_error` 에 남긴다 |

- **검색 성공이 개별 실패를 덮지 않는다.** 일시적 실패는 다음 실행에서 성공으로 덮이므로,
...
#### 수집 주기는 관리자 설정이 정하고, cron 은 확인만 한다
cron 은 `17 * * * *` 로 **매시간** 돌지만, 실제 주기는 `ai_news_settings.collection_interval_hours`(기본 2)다.
판정은 서버가 한다 — `internalConfig()` 가 마지막 실행(`ai_news_runs.started_at`)에 주기를 더해
`collectionDue` / `nextCollectionAt` 을 내려 주고, 크롤러는 그걸 보고 종료할지 정한다.

- **크롤러가 스스로 계산하지 않는 이유**: 마지막 실행 시각이 DB 에만 있고, 양쪽이 각자 계산하면 주기가 조용히 어긋난다.
- 차례가 아니면 크롤러는 **`ai_news_runs` 행을 만들기 전에** 끝낸다. 그래서 실행 이력 표의 간격이 곧 실제 수집 주기다.
  건너뛴 실행을 행으로 남기면 이력이 의미를 잃는다.
- `startRun()` 이 `LocalDateTime.now(SERVICE_ZONE)` 을 쓰는 것도 이 때문이다. 주기 계산과 시계가 달라선 안 된다.
- 크롤러의 `config.get("collectionDue", True)` 기본값은 **배포 순서 때문**이다. API 가 아직 옛 버전이어도
  크롤러는 지금까지처럼 동작한다.
- 주기를 길게 잡을수록 주종 순환(아래) 한 바퀴가 길어진다. 일일 한도는 주기와 무관하게 그대로 상한이다.

### 실행마다 주종 하나에 집중한다
Tavily 검색어 상한이 399자라 위스키·와인·꼬냑을 한 쿼리에 몰아넣으면 셋이 서로 밀어낸다.
`rotation_category`(`news_official.py`)가 관리자 설정의 `whisky/wine/cognac_ratio`(기본 60/20/20)를
10슬롯으로 바꿔 순환한다 — 10회 중 위스키 6·와인 2·꼬냑 2. 실행 순번(`run_id`)만으로 정해지므로
따로 상태를 저장하지 않는다. 일일 한도(`daily_release_limit`)는 발행이 아니라 **생성** 기준이다
(`countCreatedSince`) — 자동 발행이 없으니 발행 기준으로 세면 항상 0 이라 아무것도 막지 못한다.
...
- **검증 기준은 관리자와 동일하다.** 사용자 화면도 `useSpiritForm({ requireProductionInfo: true })` 다.
  요청 시점에 비어 오면 결국 관리자가 승인 화면에서 같은 칸을 다시 메우게 되기 때문이다.
  생산자·국가, 위스키 숙성연수/NAS 택1, 와인 빈티지 목록 구조가 모두 같은 규칙을 탄다.
- ⚠️ **사용자 화면은 에디션 1건 고정이고, 그 1건은 자동으로 시딩된다.**
  `allowMultipleVariants=false` 는 탭 바를 숨기는데 유일한 「＋ 에디션 추가」 버튼이 그 안에 있다.
  자동 시딩(`SpiritFormFields` 의 effect)을 지우면 사용자가 배치·싱글캐스크·출시연도(또는 와인)를
  고르는 순간 **입력할 칸도 빠져나올 버튼도 없이** `'에디션을 1건 이상 추가해주세요'` 에 갇힌다.
- 사용자는 생산자를 즉석에서 만들 수 없고 **승인 대기 큐**로만 넣는다(`allowPendingProducer`).
  id 가 없으므로 입력한 이름을 생산 정보로 인정한다. 생산자 요청 종류는 카테고리에 맞춰 보낸다 —
  `OTHER` 로 고정해 보내면 승인돼도 그 카테고리 생산자 목록에 나타나지 않는다.
- 제출 형태만 다르다: 사용자는 **평탄화 DTO + 멀티파트 이미지**,
...
- 사용자는 등록 요청 화면에서 주류를 검색해 고르고(위스키·와인만), 관리자는 검토 화면에서
...
- 승인 엔드포인트가 갈린다: `POST /requests/{id}/approve` (새 마스터) /
  ...
  본문은 같은 폼 페이로드다.
- 이름·생산자·국가·산지는 서버가 **마스터에서 복사**한다. 그래서 이 모드에서는 폼의 해당 칸을
  잠그고 생산 정보 카드를 숨긴다(`identityLocked`) — 고칠 수 있게 두면 고쳐도 아무 일이 안 일어난다.
...
- 회귀 방지: `npm run admin:verify-form` (에디션 자동 시딩·검증 기준·와인 상세 경로·`targetSpiritId` 제출).
